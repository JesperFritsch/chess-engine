...
fn negamax(
        ...
        && has_non_pawn_material(pos)
    {
        ...
        if let Ok(null_pos) = pos.clone().swap_turn() {
            ...
            let null_score = -negamax(
                search_ctx, &null_pos, depth - 1 - reduction, -beta, -beta + 1, ply + 1, &mut null_pv, false,
            )?;
            if null_score >= beta {
                ...
                let verify = negamax(
                    search_ctx, pos, depth - reduction, beta - 1, beta, ply, &mut verify_pv, false,
                )?;
                if verify >= beta {
                    return Some(beta);
                }
    ...
    for (move_count, m) in ordered_moves(pos, hash_move, search_ctx.killers[kply], &search_ctx.history)
        .into_iter()
        .enumerate()
    {
        ...
        let score = if move_count == 0 {
            ...
            -negamax(search_ctx, &child, depth - 1, -beta, -alpha, ply + 1, &mut child_pv, true)?
        } else {
            ...
            let mut s = -negamax(
                search_ctx, &child, depth - 1 - reduction, -alpha - 1, -alpha, ply + 1, &mut child_pv, true,
            )?;
            ...
            if reduction > 0 && s > alpha {
                s = -negamax(search_ctx, &child, depth - 1, -alpha - 1, -alpha, ply + 1, &mut child_pv, true)?;
            }
            ...
            // exact score and PV.
            if s > alpha && s < beta {
                s = -negamax(search_ctx, &child, depth - 1, -beta, -alpha, ply + 1, &mut child_pv, true)?;
            }
            s
        };
        ...
        if alpha >= beta {
            ...
            if is_quiet(m) {
                record_killer(search_ctx, kply, m);
                if let Some(from) = m.from() {
                    // Deeper cutoffs are worth more, hence depth² (a common weight).
                    search_ctx.history[from.to_usize()][m.to().to_usize()] +=
                        (depth * depth) as i32;
                }
            }
            break;
...
fn quiescence(
    ...
    for m in ordered_moves(pos, None, [None, None], &search_ctx.history) {
        if !m.is_conversion() {
            continue;
        }
        let mut child = pos.clone();
...
pub fn depth_bound_search(search_ctx: &mut SearchContext, pos: &Chess, depth: u32) -> Option<i32> {
    let mut pv = Vec::new();
    let result = negamax(search_ctx, pos, depth, -(MATE + 1), MATE + 1, 0, &mut pv, true);
    if result.is_some() {
...
/// A snapshot of the search after a completed iterative-deepening iteration,
/// handed to a progress callback so a UI can show live depth/score/PV.
...
pub fn time_bound_search(search_ctx: &mut SearchContext, pos: &Chess) -> Option<i32> {
    time_bound_search_with_progress(search_ctx, pos, &mut |_| {})
}

/// Like [`time_bound_search`], but invokes `on_progress` after each completed
/// depth. Because the search is iterative deepening, this is a natural place to
/// report "now at depth N" without any threading.
pub fn time_bound_search_with_progress(
    search_ctx: &mut SearchContext,
    pos: &Chess,
    ...
    for depth in 1.. {
        ...
        if let Some(r) = result {
            ...
            on_progress(&SearchInfo {
                depth,
                score: r,
                nodes: search_ctx.node_count,
                pv: search_ctx.pv.clone(),
